...
function loaddata(page){
	if(count != -1 && ((currentpage-1) * pagesize) >= count){
		...
		$.ajax({
	       type: "post",
	       dataType: "json",
	       url: "<c:url value='/vote/myVipRelease'/>",
		          ...
		          $.each(data.list,function(index,collection){
		        	  ...
		        	  $('#thelist').append("<li class='zhedie' data-role='collapsible' data-iconpos='right' data-shadow='false' data-corners='false'>"
	   	        	  ...
	   	        	  +"<fieldset class='myuploadclass' data-role='controlgroup' data-type='horizontal'>"
	   	  		        +"<label>详情<input type='checkbox' onclick='scanCollection("+collection.id+")'></label>"
	   	  		        +"<label>修改<input type='checkbox' onclick='updateCollection("+collection.id+")'></label>"
	   	  		        +"<label>删除<input type='checkbox' onclick='deleteCollection("+collection.id+")'></label>"
	   	  		      +"</fieldset>"
	   	        	  +"</li>");
		          });
	          }
...
function scanCollection(id){
	location.href = "<c:url value='/vote/gotoHyvip'/>?id="+id;
}

function updateCollection(id) {
	location.href = "<c:url value='/vote/gotoUpdateVipCollection?id="+id+"'/>";
}
...
function deleteCollection(id) {
	...
	$('#deleteurl').click(function(){
		$.ajax({
			type:'post',
	        url:"<c:url value='/vote/deleteVipCollection'/>?id="+id,
	        success:function(data){
	        	if(data == '0000'){
...
<div data-role="footer" data-position="fixed">
  <div data-role="navbar">
		<ul>
			...
			<li><a href="<c:url value='/redirect?p=/vipcollection/vipcoll'/>" data-iconpos="top" data-icon="star" data-ajax="false">往期回顾</a></li>
		</ul>
	</div><!-- /navbar -->
</div>
