...
function loaddata(page){
	if(count != -1 && ((currentpage-1) * pagesize) >= count){
		...
		$.ajax({
	       ...
	       data: {page:page, pageSize:pagesize, type:mainscope,id:shopid},
	       beforeSend: function() {
	    	   $('#more').html("正在加载...");
	           $('#more').show();
		   },
...
<body >
	<div data-role="page">
	<c:if test="${item.id!=7792}">
	<jsp:include page="../head/header.jsp?name=店铺详情"></jsp:include>
	</c:if>
	<c:if test="${item.id==7792}">
	<jsp:include page="../head/header.jsp?name=官方推荐"></jsp:include>
	</c:if>
	<c:if test="${item.id!=7792}">
	<div>
		  店铺名称：${item.shopname} ${sessionScope.sessionid}<hr/>
		  电话：${item.mobile}<hr/>
		  地址：${item.fullname}${item.shopaddress}<hr/>
          微信号：${item.weixin}<hr/>
		  共上传：${item.sumcollection}件藏品<hr/>
		 待售中：${item.dscollection}件藏品<hr/>
		已售罄：${item.yscollection}件藏品<hr/>
	</div>
	</c:if>
		<div id="imageid"></div>
