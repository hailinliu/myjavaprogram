	...
	<div data-role="page">
  	...
  	<div data-role="content">
		...
		<form id="uploadform" method="post" action="<c:url value='/vote/uploadCollectionMsg'/>" onsubmit="return checkForm();" enctype="multipart/form-data" data-ajax="false">
		    ...
		    <div>
		      <label>藏品名称</label>
		      <input id="nameid" type="text" name="name" value="${activity.name}" maxlength="300" placeholder="1-20个汉字" />
		    </div>
		    <div>
		      <label>图片</label>
		      ...
		      <input type="hidden" id="oldimageid" name="oldimage" value="${activity.imgurl}"/>
		    </div>
		    <div>
		      <label>藏品介绍</label>
		      <textarea name="memo" placeholder="200个汉字" rows="10">${activity.note}</textarea>
		    </div>
