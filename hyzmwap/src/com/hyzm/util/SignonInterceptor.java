...
public class SignonInterceptor extends HandlerInterceptorAdapter {
	...
	public boolean preHandle(HttpServletRequest request,
		...
		boolean flag = false;
		if(currentSession == null && (url.indexOf("user") >= 0 || url.indexOf("mine") >= 0 || url.indexOf("pay") >= 0))
			flag = true;
		if(url.indexOf("verify") >= 0 || url.indexOf("Register") >= 0 || url.indexOf("Login") >= 0 || url.indexOf("SendSms") >= 0 || url.indexOf("pay/callback") >= 0 || url.indexOf("pay/notify") >= 0)
			flag = false;
		if (flag) {
			request.setCharacterEncoding("utf-8");
			response.setCharacterEncoding("utf-8");
			...
			sb.append(request.getContextPath() + "/redirect?p=login\";</script>");
			out.print(sb.toString());
			out.close();
			return false;
		}
