...
function refreshFiles(dir) {
    ...
    $.getJSON(MANAGER_SERVLET+"?t="+new Date(),{
        ...
        $.each(data,function(entryIndex,entry){
            ...
            '" directory="'+dir+'" title="重命名"/>&nbsp&nbsp'+
            '<img src="../image/jianqie.png" class="tailorButton" file="'+entry['fileName']+
            '" directory="'+dir+'" title="裁剪"/></td>';
            html+='</tr>';
            $('#title').after(html);
        });
...
function bindClickAction() {
    ...
    $(".tailorButton").click(function(){
        ...
        var dir = $(this).attr("directory");
        //jPrompt: jquery plugin for alert dialogs et.
        jPrompt('宽度@高度(宽高之间用@分开)：', '', '裁剪', function(newwidthnewheight) {
            if (newwidthnewheight){
            	//alert(newwidthnewheight.split("@")[0]);alert(newwidthnewheight.split("@")[1]);
            	tailorFolder(dir,fileName, newwidthnewheight.split("@")[0],newwidthnewheight.split("@")[1]);
            }
        });
    });
}
...
function addFolder(parentDir, dirName) {
	/*if (dirName.indexOf('small') > -1) {
		jAlert('error', '文件名不能包含“small”');
		return;
	}*/
...
//dir是相对根目录的路径，newName只有最后一个文件夹名称
function tailorFolder(dir,fileName, newwidth,newheight) {
    $.post(MANAGER_SERVLET, {
        command:"tailorFolder",
        fileName:fileName,
        directory:dir,
        newwidth : newwidth,
        newheight : newheight
    ...
    function(data){
        if(data ==null || data['success'] != true){
            jAlert('error', "裁剪名失败，可能存在同名目录或新目录名含有非法字符(? \\ / : * \" < > |)，请重试。");
            return;
        }
        refreshFiles(dir);
    }, "json");
