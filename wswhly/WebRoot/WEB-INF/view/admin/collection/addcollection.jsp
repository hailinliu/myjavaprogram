...
function formsubmit(){
	$('#addcollectionform').form('submit', {
		url: "<c:url value='/collection/addcollection'/>",
		onSubmit: function() {
		...
		success: function(data) {
			if(data == "0000"){
				$.messager.alert('提示', "添加藏品成功！", 'info',function(){
					...
					location.href = '<c:url value="/sys/redirect?p=admin/collection/collectionManage"/>';
				});
			}else{
  ...
  <body>
	...
	<form id="addcollectionform" method="post">
	   ...
	   <table width="100%" border="0" align="center" cellpadding="10" cellspacing="10" style="font-size: 12px;padding: 5px;background-color: #efefef;border:1px solid #A4BED4;">
			<tr>
			    <td>藏品类型:</td><td>
				<input name="type" class="easyui-combobox" data-options="required:true,url:'<c:url value="/collection/typecombox"/>', valueField:'id', textField:'typename', panelHeight:'auto'">
				</td>
			</tr>
			...
			<tr>
			  <td>藏品归属:</td>
			  <td>
			     <input type="radio" checked="checked" name="owner" value="自有"/>自有&nbsp;&nbsp;
			     <input type="radio" name="owner" value="受人委托"/>受人委托&nbsp;&nbsp;
			     <input type="radio" name="owner" value="他人拥有"/>他人拥有
			  </td>
			</tr>
			<tr>
			  <td>藏品来源:</td>
			  <td>
			     <input type="radio" checked="checked" name="source" value="家传"/>家传&nbsp;&nbsp;
			     <input type="radio" name="source" value="普通买卖"/>普通买卖&nbsp;&nbsp;
			     <input type="radio" name="source" value="拍卖"/>拍卖&nbsp;&nbsp;
			     <input type="radio" name="source" value="其他方式"/>其他方式
			  </td>
			</tr>
			<tr><td>藏品简介:</td><td>
			</td>
