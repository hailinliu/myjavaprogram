                 ...
                 document.getElementById("updateadminform").reset();
                  ...
                  $('#jgnameid').empty().append(row.name);
               	  ...
               	  if(row.level==0||row.level==1){
               	   $('#jgguzhi').attr("disabled",true);
               	   $('#isguizhi').html('不提供估值服务！');
               	  }else{
...
return;
	...
	$('#messagetable').datagrid('loadData',{total:0,rows:[],footer:[]});
		...
		$('#messagetable').datagrid('load',{'name':name,'zjuserid':zjuserid,'typeid':typeid,ordercolumn:'yjresultsdate'});
	}
  ...
  <body>
    ...
    <table id="messagetable" class="easyui-datagrid" data-options="url:'<c:url value="/onlinepic/getPicCollForList"/>',queryParams:{'zjuserid':${CurrentSession.userinfo.id},'ordercolumn':'yjresultsdate'},title: '图片鉴定藏品列表',frozenColumns:[[{field:'ck',checkbox:true}]],singleSelect:true,fitColumns:true,nowrap:false,pagination:true,rownumbers:true,striped:true,showFooter:true">
		<thead>
			<tr>
