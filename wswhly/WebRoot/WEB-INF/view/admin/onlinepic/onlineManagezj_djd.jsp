          ...
          toolbar:[{
              ...
              handler:function(){
                var row = $('#messagetable').datagrid('getSelected');
                 if(row){
                 if(row.state==2||row.state==1||row.state==9){
                 document.getElementById("updateadminform").reset();
                   $('#jgguzhitr').css("display","none");
					$('#jgisshangpaitr').css("display","none");
					$('#jghangqingtr').css("display","none");
                  $('#jgnameid').empty().append(row.name);
               	  $('#jgid').val(row.id);
               	  $('#jglevel').val(row.level);
               	 $('#jgguzhi').attr("disabled",false);
               	   $('#isguizhi').html('');
             	   setNiandai(row.typeid);
               	  $('#scanmessage').window('open');
                 }else{
                  $.messager.alert('警告','该信息已审核','warning');
                 }
                 }else{
...
return;
	...
	$('#messagetable').datagrid('loadData',{total:0,rows:[],footer:[]});
		...
		$('#messagetable').datagrid('load',{'name':name,'zjuserid':zjuserid,'typeid':typeid,'state':'1',ordercolumn:'yjresultsdate'});
	}
  ...
  <body>
    ...
    <table id="messagetable" class="easyui-datagrid" data-options="url:'<c:url value="/onlinepic/getPicCollForList"/>',queryParams:{'zjuserid':${CurrentSession.userinfo.id},'ordercolumn':'yjresultsdate','state':'1'},title: '图片鉴定藏品列表',frozenColumns:[[{field:'ck',checkbox:true}]],singleSelect:true,fitColumns:true,nowrap:false,pagination:true,rownumbers:true,striped:true,showFooter:true">
		<thead>
			<tr>
...
<%--				<th data-options="field:'bh',width:20,halign:'center'">编号</th>--%>
				...
				<th  data-options="field:'pic',width:50,halign:'center',formatter:function(value,row,index){			
		...
		<div class="easyui-layout" data-options="fit:true">
			<div data-options="region:'center',border:false" style="padding:10px;background:#fff;border:1px solid #ccc;">
				...
				<tr><td>退回原因:<select id="thstate" name="thstate" class="easyui-combobox" editable="false" style="width: 170px" data-options="panelHeight:'auto',required:true">
						...
						<option value="4">该藏品所选门类与实际不符</option>
