...
$(document).ready(function(){
	...
	for (var i = 0; i < imageslist.length-1 ; i++) {
		$('#imagelist').append("<img src='"+imageslist[i]+"' width='200' onclick='deleteImg(this)' style='cursor:pointer;'/><br/>");
	}
	
	var imagestitle = '${collection.imgurltitle}';
	var imageslist1 = imagestitle.split(',');
	$('#imagelist1').empty();
	for (var i = 0; i < imageslist1.length-1 ; i++) {
		$('#imagelist1').append("<img src='"+imageslist1[i]+"' width='200' onclick='deleteImg(this)' style='cursor:pointer;'/><br/>");
	}
	
	
	$('#typeid').combobox('setValue', '${collection.type}');
	$('#styleid').combobox({ 
		url:'<c:url value="/collectionus/stylecombox?typeid=${collection.type}"/>', 
		valueField:'id', 
		textField:'typename' 
	});
	$('#styleid').combobox('setValue', '${collection.style}');
});
...
function formsubmit(){
	...
	$('#editcollectionform').form('submit', {
		url: "<c:url value='/collectionus/editcollection'/>",
		onSubmit: function() {
		...
		success: function(data) {
			if(data == "0000"){
				$.messager.alert('提示', "修改藏品成功！", 'info',function(){
					...
					window.dialogArguments.$('#collectionustable').datagrid('reload');
					window.close();
				});
			}else{
  ...
  <body>
	...
	<form id="editcollectionform" method="post">
	   ...
	   <table width="100%" border="0" align="center" cellpadding="0" cellspacing="1" style="font-size: 12px;padding: 5px;background-color: #efefef;border:1px solid #A4BED4;">
			...
			<tr><td>节目期号:</td><td><input name="tvnum" value="${collection.tvnum}" class="easyui-validatebox" type="text"></td></tr>
			<tr valign="middle">
			    <td colspan="2">
			    <script type="text/javascript">
			    	var imageid="";
