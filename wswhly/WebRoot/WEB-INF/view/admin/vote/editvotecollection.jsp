...
for (var i = 0; i < imageslist.length-1 ; i++) {
	$('#imagelistid').append("<img src='../voteactivity/small/"+imageslist[i]+"'/>");
}
});
...
function formsubmit(){
	$('#editcollectionform').form('submit', {
		url: "<c:url value='/vote/updVoteCollection'/>",
		onSubmit: function() {
		...
		success: function(data) {
			if(data == "0000"){
				$.messager.alert('提示', "修改藏品成功！", 'info',function(){
					if ('${type}' == 2) {
						window.location.href="<c:url value='/sys/redirect?p=admin/vote/voteactivityManage'/>";
				    } else if ('${type}' == 3) {
						window.location.href="<c:url value='/sys/redirect?p=admin/vote/votecollectionManage'/>";
				    } else {
  ...
  <body>
	...
	<form id="editcollectionform" method="post">
	   ...
	   <input id="imgid" type="hidden" name="imgurl" value="${collection.imgurl}"/>
	   <input type="hidden" name="issue" value="${collection.issue}"/>
