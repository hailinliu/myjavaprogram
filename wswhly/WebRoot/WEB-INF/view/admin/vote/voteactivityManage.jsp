...
$(function(){
	...
	$('#activitytable').datagrid({
		toolbar:[{
                 ...
                 iconCls:'icon-search',
				handler:function(){
				var row = $('#activitytable').datagrid('getSelected');
				if(row){
            ...
            id:'voteactivitybtnscan',
            text:'藏品详情',
            iconCls:'icon-search',
            handler:function(){
               var row = $('#activitytable').datagrid('getSelected');
               if(row){
             	  $('#scanactivitytypenameid').empty().append(row.typename);
             	  $('#scanactivitynameid').empty().append(row.name);
             	  $('#scanactivityserialnumid').empty().append(row.serialnum);
             	  $('#scanactivityusernameid').empty().append(row.username);
             	  $('#scanactivityuploaddateid').empty().append(row.uploaddate);
                  var image = row.imgurl.split(',');
             	  $('#scanactivityimgurlid').empty().append('<img src="<c:url value="/voteactivity/"/>'+image[0]+'" width="250" height="250"/>');
             	  $('#scanactivitynoteid').empty().append(row.note);
             	  $('#scanactivity').window('open');
               }else{
             	  $.messager.alert('警告','请选择一条藏品','warning');
               }
            }
           },{
            id:'voteactivitybtnupd',
            text:'修改藏品',
            iconCls:'icon-edit',
            handler:function(){
         	  var row = $('#activitytable').datagrid('getSelected');
         	  if(row){
           	      location.href = "<c:url value='/vote/gotoUpdVoteCollection'/>?id="+row.id+"&type=2";
               }else{
             	  $.messager.alert('警告','请选择一条藏品信息','warning');
               }
            }
           },{
            id:'voteactivitybtnshow',
            text:'显示/隐藏',
            iconCls:'icon-search',
            handler:function(){
        	  var row = $('#activitytable').datagrid('getSelected');
...
function serach(){
	$('#activitytable').datagrid('loadData',{total:0,rows:[]});
	...
	$('#activitytable').datagrid('load',{'issue':issue,'type':type,'serialnum':serialnum,'name':name,'username':username,'userid':userid,'issuetype':2,'state':state,'begindate':sdate,'enddate':edate});
}
</script>
  </head>
  ...
  <body>
    <table width="100%" border="0" align="center" cellpadding="0" cellspacing="1" style="font-size: 12px;padding: 5px;background-color: #efefef;border:1px solid #A4BED4;">
		...
		<td>活动期数:<input id="issueid" name="title" type="text" size="10" editable="false"/></td>
    ...
    <table id="activitytable" class="easyui-datagrid" data-options="url:'<c:url value="/vote/getVoteActivityList?issuetype=2"/>',queryParams:{'userid':'${comprehensiveid}','begindate':formatMonth(-3),'enddate':formatDay(0)},title: '我要上华豫',frozenColumns:[[{field:'ck',checkbox:true}]],singleSelect:true,fitColumns:true,nowrap:false,pagination:true,rownumbers:true,striped:true">
		<thead>
			<tr>
				<th data-options="field:'title',width:40,halign:'center'">活动名称</th>
				...
				<th data-options="field:'votenum',width:20,halign:'center',formatter:function(value,row,index){
				   return value+'票';
				}">投票数量</th>
	...
	<div id="scanactivity" class="easyui-window" data-options="title:'藏品详情',iconCls:'icon-search',modal:true" style="width:700px;height:500px;padding:5px;background: #fafafa;overflow:auto;">
	   <div class="easyui-layout" data-options="fit:true">
		<div data-options="region:'center',border:false" style="padding:10px;background:#fff;border:1px solid #ccc;">
			<table id="oneactivitytable" width="100%" cellpadding="5" cellspacing="1">
			  <tr>
			    <td width="30%">所属类型：</td>
			    <td id="scanactivitytypenameid"></td>
			    <td id="scanactivityimgurlid" rowspan="5" width="250" height="250"></td>
			  </tr>
			  <tr>
			    <td>藏品名称：</td>
			    <td id="scanactivitynameid"></td>
			  </tr>
			  <tr>
			    <td>编号：</td>
			    <td id="scanactivityserialnumid"></td>
			  </tr>
			  <tr>
			    <td>上传者：</td>
			    <td id="scanactivityusernameid"></td>
			  </tr>
			  <tr>
			    <td>上传日期：</td>
			    <td id="scanactivityuploaddateid"></td>
			  </tr>
			  <tr>
			    <td colspan="3"><u id="scanactivitynoteid"></u></td>
			  </tr>
			</table>
		</div>
		<div data-options="region:'south',border:false" style="text-align:right;padding:5px 0;">
			<a class="easyui-linkbutton" data-options="iconCls:'icon-cancel'" href="javascript:void(0)" onclick="$('#scanactivity').window('close');">关闭</a>
		</div>
	  </div>
	</div>
