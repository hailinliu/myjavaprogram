...
public class VideoControl_back {
	...
	public void addVideo(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			String videotype=request.getParameter("videotype");
			if (videoService.addVideo(title, videoname+".jpg", videoname+".flv", proficient, note,videotype)) {
				out.print("0000");
	...
	public void editVideo(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			out = response.getWriter();
			long id = Long.valueOf(request.getParameter("id"));
			Map<String, Object> map = videoService.getVideoById(id);
			String title = request.getParameter("title");
			String videoname = request.getParameter("videoname");
			if (videoname == null || "".equals(videoname)) {
				videoname = map.get("name").toString();
			} else {
				File updFile = new File(saveFilePath + map.get("name").toString());
				File imgFile = new File(saveFilePath + map.get("imgurl").toString());
				if (updFile.exists()) {
					updFile.delete();
				}
				if (imgFile.exists()) {
			...
			String note = request.getParameter("note");
			String videotype=request.getParameter("videotype");
			if (videoService.updVideo(id, title, videoname+".jpg", videoname+".flv", proficient, note,videotype)) {
				out.print("0000");
