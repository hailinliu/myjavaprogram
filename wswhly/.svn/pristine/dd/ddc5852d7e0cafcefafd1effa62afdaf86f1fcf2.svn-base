...
public class ImageUtil {
	...
	public static void imageZip(String realpath, String imgname, String extensionName, int maxWidth, int maxHeight) {
		ImageUtil is = new ImageUtil();
		try {
			//String oldsrc = "./collection/" + type + "/" + imgname + ".jpg";
			//String newsrc = "./collection/" + type + "small" + "/" + imgname + ".jpg";
			String oldsrc = realpath + imgname + "." + extensionName;
			String newsrc = realpath + "small" + "/" + imgname + "." + extensionName;
			is.saveImageAsJpg(oldsrc,newsrc, maxWidth, maxHeight);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	public void saveImageAsJpg(String fromFileStr, String saveToFileStr,
		...
		if (!saveFile.exists()) {//判断路径是否存在，如果不存在创建
			saveFile.mkdirs();
		}
