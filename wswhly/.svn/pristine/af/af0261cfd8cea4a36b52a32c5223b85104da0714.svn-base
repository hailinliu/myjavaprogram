...
$(function(){
	...
	$('#newstable').datagrid({
          toolbar:[{
              ...
              iconCls:'icon-add',
              handler:function(){
                 location.href = "<c:url value='/news/gotoaddschoolInfo'/>";
              } 
             },{
              id:'schoolinfobtnedit',
              text:'修改学院下属资讯',
              iconCls:'icon-edit',
              handler:function(){
           	  var row = $('#newstable').datagrid('getSelected');
           	  if(row){
           		    if (row.state == 2){
               	      ...
               	      location.href = "<c:url value='/news/gotoeditschoolInfo'/>?id="+row.id;
   		            }
                 }else{
               	  $.messager.alert('警告','请选择一条信息','warning');
                 }
              ...
              handler:function(){
                 ...
                 handler:function(){
           	      $('#typetable').datagrid({
    				    url:"<c:url value='/news/newstypelist'/>?parentid=2",
    				    title:'学院类型列表',
    				    fitColumns:true,
    				    rownumbers:true,
    				    striped:true,
	...
	$('#titleid').autocomplete('<c:url value="/news/autotitle"/>?type=2', 
		      {
		      minChars: 0, 
		      width :400, 
		      dataType: "json",
	...
	function serach(){
		...
		$('#newstable').datagrid('load',{'title':title,'recommend':recommend,'state':state,'begindate':sdate,'enddate':edate,'type':2,'style':style});
	}
...
function delType(id){
  ...
  <body>
    <table width="100%" border="0" align="center" cellpadding="0" cellspacing="1" style="font-size: 12px;padding: 5px;background-color: #efefef;border:1px solid #A4BED4;">
		...
		<td>学院标题:<input id="titleid" name="title" type="text" size="10" editable="false"/></td>
		<td>资讯类型:<input id="styleid" name="style" class="easyui-combobox" data-options="url:'<c:url value="/news/gettypecombobox"/>?parentid=2', valueField:'style', textField:'style', panelHeight:'auto'"/></td>
    ...
    <table id="newstable" class="easyui-datagrid" data-options="url:'<c:url value="/news/getnewslist"/>',queryParams:{'begindate':formatMonth(-3),'enddate':formatDay(0),'type':2},title: '新闻列表',frozenColumns:[[{field:'ck',checkbox:true}]],singleSelect:true,fitColumns:true,nowrap:false,pagination:true,rownumbers:true,striped:true,showFooter:true">
		<thead>
			<tr>
				...
				<th data-options="field:'stylename',width:35,halign:'center'">资讯类型</th>
	...
	<!-- 查看新闻详细内容 -->
	<div id="scannews" class="easyui-window" data-options="title:'新闻详情',iconCls:'icon-search',modal:true" style="width:700px;height:500px;padding:5px;background: #fafafa;overflow:auto;">
	...
	<!-- 学院资讯类型 -->
	<div id="newstype" class="easyui-window" data-options="title:'学院资讯类型',iconCls:'icon-search',modal:true" style="width:500px;height:500px;padding:5px;background: #fafafa;overflow:auto;">
		<div class="easyui-layout" data-options="fit:true">
			<div data-options="region:'center',border:false" style="padding:10px;background:#fff;border:1px solid #ccc;">
	...
	<!-- 添加类型 -->
	<div id="addtype" class="easyui-window" data-options="title:'添加学院资讯类型',iconCls:'icon-add',modal:true" style="width:300px;height:150px;padding:5px;background: #fafafa;overflow:auto;">
		<div class="easyui-layout" data-options="fit:true">
			<div data-options="region:'center',border:false" style="padding:10px;background:#fff;border:1px solid #ccc;">
	...
	<!-- 修改类型 -->
	<div id="edittype" class="easyui-window" data-options="title:'修改学院资讯类型',iconCls:'icon-edit',modal:true" style="width:300px;height:150px;padding:5px;background: #fafafa;overflow:auto;">
		<div class="easyui-layout" data-options="fit:true">
			<div data-options="region:'center',border:false" style="padding:10px;background:#fff;border:1px solid #ccc;">
