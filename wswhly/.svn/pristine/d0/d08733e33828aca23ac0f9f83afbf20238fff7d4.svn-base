...
return;
	...
	$('#messagetable').datagrid('loadData',{total:0,rows:[],footer:[]});
		...
		$('#messagetable').datagrid('load',{'name':name,'zjuserid':zjuserid,'typeid':typeid,'state':'9',ordercolumn:'yjresultsdate'});
	}
  ...
  <body>
    ...
    <table id="messagetable" class="easyui-datagrid" data-options="url:'<c:url value="/onlinepic/getPicCollForList"/>',queryParams:{'zjuserid':${CurrentSession.userinfo.id},'ordercolumn':'yjresultsdate','state':'9'},title: '图片鉴定藏品列表',frozenColumns:[[{field:'ck',checkbox:true}]],singleSelect:true,fitColumns:true,nowrap:false,pagination:true,rownumbers:true,striped:true,showFooter:true">
		<thead>
			<tr>
