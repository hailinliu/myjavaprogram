	...
	$('#collectiontable').datagrid({
		toolbar:[{
            ...
            handler:function(){
               ...
               if(row){
                   ...
                   var image = row.imgurl.split(',');
              	  $('#scancollectionimgurlid').empty().append('<img src="<c:url value="/voteactivity/"/>'+image[0]+'" width="250" height="250"/>');
              	  $('#scancollectionnoteid').empty().append(row.note);
            ...
            handler:function(){
         	  var row = $('#collectiontable').datagrid('getSelected');
         	  if(row){
           	      location.href = "<c:url value='/vote/gotoUpdVoteCollection'/>?id="+row.id+"&type=3";
               }else{
             	  $.messager.alert('警告','请选择一条藏品信息','warning');
               }
            }
           },{
            id:'votecollectionbtnshow',
            text:'显示/隐藏',
            iconCls:'icon-search',
            handler:function(){
        	  var row = $('#collectiontable').datagrid('getSelected');
...
function serach(){
	...
	$('#collectiontable').datagrid('load',{'issue':issue,'type':type,'serialnum':serialnum,'name':name,'username':username,'userid':userid,'issuetype':3,'state':state,'begindate':sdate,'enddate':edate});
}
</script>
  </head>
  ...
  <body>
    ...
    <table id="collectiontable" class="easyui-datagrid" data-options="url:'<c:url value="/vote/getVoteCollectionList?issuetype=3"/>',queryParams:{'userid':'${comprehensiveid}','begindate':formatMonth(-3),'enddate':formatDay(0)},title: '藏品征集令',frozenColumns:[[{field:'ck',checkbox:true}]],singleSelect:true,fitColumns:true,nowrap:false,pagination:true,rownumbers:true,striped:true">
		<thead>
			<tr>
		...
		<div data-options="region:'center',border:false" style="padding:10px;background:#fff;border:1px solid #ccc;">
			<table id="onecollectiontable" width="100%" cellpadding="5" cellspacing="1">
			  <tr>
			    ...
			    <td id="scancollectionimgurlid" rowspan="5" width="250" height="250"></td>
			  </tr>
			  <tr>
			    <td>藏品名称：</td>
