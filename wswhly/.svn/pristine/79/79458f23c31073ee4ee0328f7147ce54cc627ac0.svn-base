...
var settings = {
       ...
       upload_url: "<c:url value='/email/uploademail'/>",
...
function formsubmit(){
	$('#addeamilform').form('submit', {
		url: "<c:url value='/email/sendEmail'/>",	
		onSubmit: function() {
		...
		success: function(data) {
			if(data == "0000"){
				$.messager.alert('提示', "邮件发送成功！", 'info',function(){
					document.getElementById("addeamilform").reset();
					location.href = '<c:url value="/sys/redirect?p=admin/emali/emailInfoManage"/>';
				});
			}else{
  ...
  <body>
	<form id="addeamilform" method="post">
	   <input id="eamilid" type="hidden" name="eamil"/>
	   <table width="100%" border="0" align="center" cellpadding="10" cellspacing="10" style="font-size: 12px;padding: 5px;background-color: #efefef;border:1px solid #A4BED4;">	
			<tr><td>邮件主题:</td><td><input name="subject" class="easyui-validatebox" type="text" data-options="required:true"></td></tr>
