package com.wswhly.util.sendmail;
        ...
        try {
            ...
            if (files != null && files.size() > 0) {
                for (File file : files) {
                    ...
                    //attachmentBodyPart.setFileName(fds.getName()); // 锟矫碉拷锟侥硷拷锟斤拷同锟斤拷锟斤拷锟斤拷BodyPart
   ...
   public static void createImage(String str,Font font,File outFile) throws Exception{
    ...
    public static void main(String[] args) {
        ...
        for(int i=0;i<usermail.size();i++){
        	boolean tag= SendMail.doSendHtmlEmail("2014鏈�", str, usermail.get(i), filelist);//
        	if(tag){
        		System.out.println(usermail.get(i)+":鎴愬姛锛�");
        	}else{
        		System.out.println(usermail.get(i)+":澶辫触锛�");
        	}
        }
    }
