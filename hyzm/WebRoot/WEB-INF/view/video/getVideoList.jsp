...
<html>
  ...
  <script type="text/javascript" src="<c:url value='/js/jquery_date.js'/>"></script>
	<meta name=keywords content="专家讲坛，专家视频，专家讲座">
	<meta name=description content="华豫之门专家讲坛">
  </head>
...
<div class="main_box clear">
  ...
  <div class="mian_right clear">
     <div class="qjt_pic"><span class="bread_nav">当前位置：首页<span class="jt">></span>专家讲坛</span></div>
     ...
     <c:forEach items="${videoList}" var="video" varStatus="vdn">
       <c:if test="${vdn.count % 3 == 1}">
           <ul class="pic_list3 clear">
		      ...
		      <li>
		           <a href="<c:url value='/video/${video.id}.dhtml'/>" target="_blank"><img src="${video.imgurl}" width="235" height="216" alt="${fn:substring(video.title,0,18)}" /></a>
		            <div class="tab2 clear">
		               <ul>
		                  <li><a href="<c:url value='/video/${video.id}.dhtml'/>" target="_blank">视频名称：<c:choose><c:when test="${fn:length(video.title) > 18}">
