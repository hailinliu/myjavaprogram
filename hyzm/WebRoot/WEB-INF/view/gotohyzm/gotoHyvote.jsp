...
<%@page import="com.hyzm.bean.vote.ActivityListRespBean.Activity"%>
...
<html>
  <head>
    ...
    <title>${activity.name}_卫视文化乐园-华豫之门官方网站</title>
...
<div class="main_box clear">
   <div class="scpjs_pic">
      <span class="bread_nav">当前位置：首页<span class="jt">></span><a href="<c:url value='/vote/activityAllList'/>">我要上华豫</a> <span class="jt">></span><span class="jt">${activity.name}</span> </span>
   </div>
   <div class="scpjs_nr clear">
     <img src="<%=PropertiesUtil.getPropertiesValue("uploadVoteurl")+"/"+images[0]%>" alt="${activity.name}" alt="${activity.name}"/>
     <div class="scpjs_list">
       ...
       <p class="comments_box1 fl" style="width:730px;">
   ...
   <div style="text-align:right;margin-bottom:20px;" >
   		<c:if test="${activity.updatenum < 3 && not empty CurrentSession && CurrentSession.username eq activity.uploadusername}"><a href="javascript:void(0)" class="bianji" onclick="javascript:$('#buch_text').show();">我要补充</a></c:if>
   		<!--  <div style="width:1035px;height:1px;background:#E6E6E6;position:absolute;top:10px;left:0;"></div>	 -->
   </div>

   <!-- 补充弹出层 -->
   <div class="forgebox_guowai" id="buch_text">
       ...
       <p></p>
	     ...
	     <p align="center">
	        <%
       	    ...
       	    %>
		        <img src="<%=PropertiesUtil.getPropertiesValue("uploadVoteimg")+"/"+str%>" width="800" alt="${activity.name}" alt="${activity.name}"/>
       	    <%
	           }
	        %>
	     </p>
